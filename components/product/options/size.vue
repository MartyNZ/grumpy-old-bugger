...
// console.log("Selected Value ID: ", props.selectedOptionValue.id);
...
<template>
  <div class="border-b-surface-800 dark:border-b-surface-300 my-2 flex items-center gap-2 border-b py-2">
    <p>{{ option.name }}</p>
    <div class="flex flex-wrap gap-2">
      <div v-for="value in option.values" :key="value.id" @click="selectNewValueId(value.id)" :class="[
        'size-selector w-content flex items-center justify-center gap-2 rounded-lg px-2 py-1 text-sm',
