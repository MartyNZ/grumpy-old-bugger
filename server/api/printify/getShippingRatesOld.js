export default defineEventHandler(async (event) => {
  const { printifyBearerToken } = useRuntimeConfig();
  ...
  // console.log(JSON.stringify(itemsByProvider));
  ...
  for (const pp_id in itemsByProvider) {
    ...
    try {
      const response = await fetch(
        `${process.env.NUXT_PUBLIC_SITE_URL}/api/printify/shipping?bp_id=${bp_id}&pp_id=${pp_id}`,
        {
          headers: {
  ...
  // console.log(JSON.stringify(shippingByProvider));
